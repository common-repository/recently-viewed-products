...
if( !class_exists('rvp_view_list')){
	class rvp_view_list{
		...
		public function rvp_view_shortcode(){
			ob_start();
			$this->rvp_view_single();
			return ob_get_clean();
		}

		public function rvp_view_single_short($no_of_prods,$wrap_slider){
			ob_start();
	      ...
	      'slides_to_show' => $no_of_prods
			...
			if(is_user_logged_in()){
				$this->rvp_view_logged_in($no_of_prods,$wrap_slider);
			} else {
				$this->rvp_view_guest($no_of_prods,$wrap_slider);
			}
			return ob_get_clean();
		}
